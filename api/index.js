...
app.get([
  ...
  try {
    ...
    if (!customFaviconUrl || !customLogoUrl) {
      const staticData = getStaticData();
      if (staticData && staticData.mockSettings) {
        if (!customFaviconUrl) customFaviconUrl = staticData.mockSettings.favicon_url || '';
        if (!customLogoUrl) customLogoUrl = staticData.mockSettings.logo_url || '';
      }
    }
