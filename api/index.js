...
app.get(['/sitemap.xml', '/sitemap', '/api/sitemap.xml'], (req, res) => {
  ...
  const staticRoutes = ['/', '/new-apps', '/news', '/blogs', '/videos', '/about', '/developers', '/contact', '/privacy', '/report-removal', '/terms', '/responsibility', '/notice', '/ethics', '/disclaimer'];
  ...
  if (fs.existsSync(backupPath)) {
    try {
      ...
      for (const app of data.apps || []) {
        if (app.slug) xml += `  <url>\n    <loc>${host}/app/${escapeHtmlForSitemap(app.slug)}</loc>\n    <lastmod>${getFormattedDate(app)}</lastmod>\n    <changefreq>weekly</changefreq>\n    <priority>0.9</priority>\n  </url>\n`;
      }
      ...
      for (const item of data.blogs || []) {
        if (item.slug) xml += `  <url>\n    <loc>${host}/blog/${escapeHtmlForSitemap(item.slug)}</loc>\n    <lastmod>${getFormattedDate(item)}</lastmod>\n    <changefreq>weekly</changefreq>\n    <priority>0.7</priority>\n  </url>\n`;
      }
