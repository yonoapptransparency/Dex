...
function PublicRouteErrorFallback({ onReset }: { onReset?: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-20 min-h-[40vh] text-center px-4 max-w-md mx-auto">
      ...
      <h2 className="text-xl font-bold text-zinc-900 dark:text-zinc-100 mb-2">Unable to Load Section</h2>
      ...
      <div className="flex flex-wrap items-center justify-center gap-3">
        <button
          ...
          className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-semibold text-xs transition-all shadow-sm"
        >
          Refresh Page
        </button>
        <Link
...
class ErrorBoundary extends React.Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  render() {
    if (this.state.hasError) {
      ...
      return this.props.fallback || <PublicRouteErrorFallback onReset={this.reset} />;
    }
...
import { AppDetails, preloadAppDetails } from './lib/preloadHelper';
...
function AppContent() {
  ...
  // Pre-warm AppDetails chunk in idle time so clicking any app transitions instantly
  useEffect(() => {
        ...
        <div className="w-full h-full">
          <ErrorBoundary key={location.pathname}>
            <Routes location={location}>
              ...
              <Route path="/app/:slug" element={<Suspense fallback={<AppDetailsSkeleton />}><AppDetails /></Suspense>} />
              <Route path="/app/:slug/*" element={<Suspense fallback={<AppDetailsSkeleton />}><AppDetails /></Suspense>} />
              <Route path="/moreinfo/:slug" element={<Suspense fallback={<LoadingScreen />}><GatewayPage /></Suspense>} />
              <Route path="/info/:slug" element={<Suspense fallback={<LoadingScreen />}><GatewayPage /></Suspense>} />
              <Route path="/gateway/:slug" element={<Suspense fallback={<LoadingScreen />}><GatewayPage /></Suspense>} />
              <Route path="/download/:slug" element={<Suspense fallback={<LoadingScreen />}><GatewayPage /></Suspense>} />
              <Route path="/moredetail/:slug" element={<Suspense fallback={<LoadingScreen />}><GatewayPage /></Suspense>} />
              <Route path="/s/:slug" element={<Suspense fallback={<LoadingScreen />}><SafetyStatus /></Suspense>} />
