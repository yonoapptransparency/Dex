...
import { Menu, Shield, ShieldCheck, Info, ArrowRight, X, LayoutGrid, Newspaper, Sparkles, Send, MoreHorizontal, Search, Video, Star, Facebook, Instagram, Twitter, Linkedin, Youtube, Bot, Users, Trash2 } from 'lucide-react';
  ...
  return (
    <>
      <motion.header 
        ...
        className={`sticky top-0 z-50 transition-all duration-300 ease-out backdrop-saturate-200 backdrop-blur-xl ${
          scrolled 
            ? 'bg-white/50 dark:bg-black/50 border-b border-black/10 dark:border-white/10 shadow-[0_4px_30px_rgba(0,0,0,0.03)] py-2.5' 
            : 'bg-white/25 dark:bg-black/25 border-b border-white/20 dark:border-white/10 py-3.5'
        }`}
      >
...
function AppContent() {
  ...
  useEffect(() => {
    ...
    if (path.startsWith('/app/')) {
      ...
      const prefix = path.startsWith('/info/') ? '/info/' : (path.startsWith('/moreinfo/') ? '/moreinfo/' : '/moredetail/');
