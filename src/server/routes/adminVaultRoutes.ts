...
adminVaultRouter.post("/api/v1/admin/sync-local", verifyAdminToken, async (req: any, res) => {
  ...
  try {
    ...
    try {
      ...
      safeBackupApps = JSON.parse(JSON.stringify(finalApps)).map((app: any) => {
        delete app.encrypted_download_url;
        delete app.download_url;
        return app;
      });

      // Synchronize community review counts into each app's review_count and reviews properties
      try {
        const { communityStore } = require('../services/communityStoreService');
        if (communityStore && typeof communityStore.getAllAppsReviewStatsMap === 'function') {
          const statsMap = await communityStore.getAllAppsReviewStatsMap();
          if (statsMap && statsMap.size > 0) {
            safeBackupApps.forEach((app: any) => {
              ...
              const appStats = statsMap.get(idKey) || (slugKey && statsMap.get(slugKey)) || (nameKey && statsMap.get(nameKey));
              if (appStats && appStats.totalReviews > 0) {
                app.reviews = appStats.totalReviews;
                app.review_count = appStats.totalReviews;
              }
            });
            ...
            console.log(`[SERVER] Successfully synced authentic community review counts for apps during sync-local.`);
          }
        }
      } catch (revSyncErr) {
        console.warn('[SERVER] Could not sync app review stats in sync-local:', revSyncErr);
      }
