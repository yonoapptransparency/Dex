...
return { 
  ...
  public getAppStatsSync(appIdentifier: string, fallbackRating = 4.8, appTitle?: string, appSlug?: string) {
    const aliasKeys = this.getAliasKeysForApp(appIdentifier, appTitle, appSlug);
    const matchedApp = findAppInCatalog(appIdentifier) || (appSlug ? findAppInCatalog(appSlug) : null) || (appTitle ? findAppInCatalog(appTitle) : null);
    ...
    // Real authentic stats from catalog
    const baseTotal = matchedApp?.review_count 
      ? Number(matchedApp.review_count) 
      : (matchedApp?.reviews 
          ? Number(matchedApp.reviews) 
  ...
  /**
   * Universal Review Stats Aggregation Map:
   * Queries real community reviews directly from live Firestore / memory
   * and maps them by appId, appSlug, and appName so every app in the static
   * sync has its exact, authentic community review count.
   */
  public async getAllAppsReviewStatsMap(): Promise<Map<string, { totalReviews: number; averageRating: number; starCounts: Record<string, number> }>> {
    const result = new Map<string, { totalReviews: number; averageRating: number; starCounts: Record<string, number> }>();
    const allReviewsList: Array<{ id: string; appId: string; appSlug: string; appName: string; rating: number }> = [];

    // 1. Fetch live review records from Firestore reviews collection
    try {
      ...
      const firestoreDocs: any[] = await new Promise((resolve) => {
        const req = https.request(`https://firestore.googleapis.com/v1/projects/${cfg.projectId}/databases/${cfg.firestoreDatabaseId}/documents:runQuery?key=${encodeURIComponent(cfg.apiKey)}`, {
          method: 'POST',
      ...
      if (Array.isArray(firestoreDocs)) {
        firestoreDocs.forEach((item: any) => {
          if (!item?.document?.fields) return;
          ...
          if (docId && this.deletedReviewIds.has(docId)) return;

          const appId = f.appId?.stringValue || '';
          const appSlug = f.appSlug?.stringValue || '';
          const appName = f.appName?.stringValue || '';
          const rating = f.rating ? (parseInt(f.rating.integerValue || f.rating.stringValue, 10) || 5) : 5;
          allReviewsList.push({ id: docId, appId, appSlug, appName, rating });
        });
      }
    } catch (fetchErr) {
      console.warn('[CommunityStore] getAllAppsReviewStatsMap remote query notice:', fetchErr);
    }

    // 2. Also merge in-memory reviews
    if (this.reviews.size > 0) {
      this.reviews.forEach((r) => {
        if (r.status && r.status !== 'published' && r.status !== 'approved') return;
        if (this.deletedReviewIds.has(r.id)) return;
        if (!allReviewsList.some(item => item.id === r.id)) {
          allReviewsList.push({
            id: r.id,
            appId: r.appId,
            appSlug: r.appSlug || '',
    ...
    // 3. Aggregate reviews per catalog app
    const appStatsById = new Map<string, {
      totalReviews: number;
      ratingSum: number;
      starCounts: Record<string, number>;
      aliases: Set<string>;
    ...
    for (const r of allReviewsList) {
      const id = String(r.appId || '').trim();
      const slug = String(r.appSlug || '').trim();
      const name = String(r.appName || '').trim();

      const matchedApp = findAppInCatalog(id) || (slug ? findAppInCatalog(slug) : null) || (name ? findAppInCatalog(name) : null);
      const primaryKey = matchedApp ? String(matchedApp.id).toLowerCase().trim() : (id || slug || name).toLowerCase().trim();
      if (!primaryKey) continue;

      if (!appStatsById.has(primaryKey)) {
        const aliases = new Set<string>();
        aliases.add(primaryKey);
        if (matchedApp) {
          if (matchedApp.id) aliases.add(String(matchedApp.id).toLowerCase().trim());
          if (matchedApp.slug) aliases.add(String(matchedApp.slug).toLowerCase().trim());
          if (matchedApp.name) aliases.add(String(matchedApp.name).toLowerCase().trim());
        }
        if (id) aliases.add(id.toLowerCase());
        if (slug) aliases.add(slug.toLowerCase());
        if (name) aliases.add(name.toLowerCase());

        appStatsById.set(primaryKey, {
          totalReviews: 0,
          ratingSum: 0,
      ...
      const st = appStatsById.get(primaryKey)!;
      st.totalReviews += 1;
      st.ratingSum += r.rating;
      const starKey = String(Math.min(5, Math.max(1, Math.round(r.rating))));
      st.starCounts[starKey] = (st.starCounts[starKey] || 0) + 1;
    }

    // 4. Populate result map with all aliases for instant O(1) matching
    appStatsById.forEach((st) => {
      const avg = st.totalReviews > 0 ? parseFloat((st.ratingSum / st.totalReviews).toFixed(1)) : 4.8;
      const statRecord = {
        totalReviews: st.totalReviews,
        averageRating: avg,
        starCounts: st.starCounts
      };
      st.aliases.forEach(alias => {
        result.set(alias, statRecord);
      });
    });
