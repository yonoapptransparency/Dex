...
function AppContent() {
  ...
  // Preload primary route bundles for instant navigation
  useEffect(() => {
    const idleCb = typeof window !== 'undefined' && (window as any).requestIdleCallback
      ? (window as any).requestIdleCallback
      : (cb: () => void) => setTimeout(cb, 50);
    idleCb(() => {
      try {
        pageFactories.NewsPage();
        pageFactories.NewsDetailPage();
        ...
        <Suspense fallback={<LoadingScreen />}>
          <div className="w-full h-full">
              <Routes location={location}>
                ...
                <Route path="/app/:slug" element={<NavigateWithSlug prefix="/" />} />
