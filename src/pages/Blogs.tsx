          ...
          {mockBlogs.map(blog => (
            ...
            >
              <Link to={`/blog/${encodeURIComponent(blog.slug || blog.id)}`} className="block h-64 sm:h-80 rounded-[24px] overflow-hidden mb-6 shadow-sm border border-black/5">
                <img 
                  src={blog.cover_url || `https://images.unsplash.com/photo-1550745165-9bc0b252726f?w=800&h=400&fit=crop`} 
                  alt={blog.title} 
                  loading="lazy" 
                  decoding="async"
                  width={800} 
                  ...
                  onError={(e) => {
                    (e.target as HTMLImageElement).src = `https://images.unsplash.com/photo-1550745165-9bc0b252726f?w=800&h=400&fit=crop`;
                  }}
                />
              </Link>
