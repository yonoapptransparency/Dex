  ...
  return (
    <div className="animate-fade-in max-w-[1550px] mx-auto px-3 sm:px-6 md:px-10 plain-content mb-20">
      ...
      >
        ...
        {newsItem.logo_url && (
            <div className="w-full mb-12 rounded-xl sm:rounded-2xl overflow-hidden shadow-xs border border-black/10 dark:border-white/10 bg-zinc-100 dark:bg-zinc-900 flex items-center justify-center p-1 sm:p-2">
                <img 
                  src={newsItem.logo_url || 'https://images.unsplash.com/photo-1542751371-adc38448a05e?w=800&q=80'} 
                  alt={newsItem.title} 
                  loading="eager" 
                  fetchPriority="high" 
                  decoding="async" 
                  className="w-full h-auto max-h-[520px] object-contain rounded-lg sm:rounded-xl block mx-auto" 
                />
            </div>
        )}
